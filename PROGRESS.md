...
- Fixed the "wide browser crops the bottom off images" problem. Cause: the homepage hero sized its height purely off viewport height (`86vh`, capped 420-940px) while width always filled the screen, so on a wide monitor the box got progressively wider and shorter, forcing `object-fit: cover` to crop harder top and bottom. Fixed with `height: min(max(clamp(420px, 86vh, 940px), 56.25vw), 1100px)` — the box now can't get shorter than a 16:9 ratio relative to its own width, with an 1100px ceiling so it doesn't balloon into an oversized hero on ultrawide monitors either. Project page carousels/plates got a `max-height: 82vh` added to their existing 4:3 ratio for the same reason (huge in absolute pixels on wide screens, even though their crop ratio itself doesn't change). Also added an optional `focus` field to `images[]`/`exhibitionViews[]` in the projects schema (CSS `object-position`, e.g. `"center top"`) so any crop that still looks wrong after this can be nudged per photo — wired into both the project carousel and the homepage hero, left unset everywhere for now since the default centre crop is fine on the current photos. Build tested, confirmed in both bundled stylesheets.
