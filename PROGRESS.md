...
- Ran a full SEO audit of the site as built so far. Fixed the four critical issues found:
  - `og-default.jpg` didn't exist — every page without its own cover image (home, about, both index pages) was link-previewing with a broken image. Generated one from the Wasted On The Young hero photo (`woty-01-suit-bottle.png`, centre-cropped to 1200x630) as a placeholder until James has a proper social-share image.
  - The Workers preview URL was fully indexable with no `robots.txt` and no noindex anywhere — real risk of Google indexing `website.jameswilliamkendall.workers.dev` as a separate, duplicate site once the real domain goes live. Added `NOINDEX_SITE` flag to `lib/flags.ts` (on by default), which adds a sitewide `<meta name="robots" content="noindex, nofollow">` in `BaseLayout.astro` and a `robots.txt` endpoint (`src/pages/robots.txt.ts`) that disallows everything while it's on. Flip to `false` once james-kendall.co.uk is live and pointed here — same pattern as `SHOW_DRAFTS`.
  - Added a `noindex` prop to `BaseLayout` wired to each collection's `draft` field on project/commercial/writing single pages, so once `NOINDEX_SITE` is off, individual unfinished pages (e.g. Best Before's "Project description goes here." placeholder) still won't get indexed until their `draft` flag is cleared.
  - Homepage had zero heading tags — not even an h1. Added a visually-hidden h1 (`James Kendall — Photographer & Writer, Brighton`, using a new `.sr-only` utility class in `global.css`) plus a proper page title and meta description; previously it inherited the generic sitewide default.
- Build tested: confirmed `og-default.jpg` present in `dist`, `robots.txt` outputs `Disallow: /`, homepage has the h1/title/description, and a draft project page (`best-before`) carries the noindex meta tag.
- Remaining SEO items from the audit, not yet actioned: writing article meta descriptions over ~160 characters will get truncated by Google (several run 200+); 3 migrated writing articles still have literal "summary to be written" placeholder text as their live meta description; no structured data (Person/Article JSON-LD) anywhere; homepage hero uses plain `<img>` with no width/height (minor layout-shift risk). Flagged for a follow-up pass.

## Done — 22 August 2026 (carousel scroll jump, take two)

- The scroll-pin fix below wasn't enough on its own — James confirmed it was still jumping to the top after the fix had been live 5-10 minutes. Re-diagnosed: the real problem was structural, not just a scroll-anchoring quirk to patch around in JS. `.carousel-frame` had no fixed height at all, so it sized itself to whatever the current slide's image rendered at — different photos (different orientations) genuinely produced different frame heights, which shifted the whole page below the carousel on every slide change. No JS pin can fully paper over an actual layout shift. Fixed properly in `Carousel.astro`: gave `.carousel-frame` an explicit fixed `height: clamp(360px, 78vh, 800px)` that never varies between slides, added `align-items: center` to vertically centre shorter images within it, kept `overflow: hidden` as a safety net for a rare long two-line caption, and trimmed the image's own `max-height` to `clamp(300px, 66vh, 680px)` so there's always headroom left for the caption inside the fixed frame. Kept the earlier JS scroll-pin and `overflow-anchor: none` in place as belt and braces. Build tested, confirmed `height:clamp(360px,78vh,800px)` present in the bundled CSS on both a project page and Portraits.
...
- Built out Made Beds with real content: 9 plates from `Website Content/Made Beds/` copied to `src/assets/images/made-beds/`, same gallery/carousel pattern as Unmade Beds and Wasted On The Young, Hahnemühle Photorag captions per James's text. Added a new `companionProject` field to the projects schema (a project slug, e.g. Made Beds <-> Unmade Beds) and a matching "Companion Project" callout on the project page template (same MetaLine + big link pattern the Writing page already uses for its "Related Project" block) — set on both Made Beds and Unmade Beds so each links to the other. Left `draft: true`, matching the other real-but-unlaunched projects. Build tested, confirmed both pages link to each other and all 9 plates render.
